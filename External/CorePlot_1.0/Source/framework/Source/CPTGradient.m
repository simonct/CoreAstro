...
-(CPTGradient *)removeColorStopAtIndex:(NSUInteger)index
{
	...
	if ( isnan(removedElement.position) ) {
		[NSException raise:NSRangeException format:@"-[%@ removeColorStopAtIndex:]: index (%u) beyond bounds", [self class], index];
	}
...
-(CGColorRef)newColorStopAtIndex:(NSUInteger)index
{
	...
	[NSException raise:NSRangeException format:@"-[%@ colorStopAtIndex:]: index (%u) beyond bounds", [self class], index];
