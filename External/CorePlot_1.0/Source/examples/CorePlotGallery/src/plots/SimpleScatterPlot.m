...
//	}
	...
	if ( plotData == nil ) {
        ...
        NSArray* hist = [NSArray arrayWithContentsOfFile:@"/tmp/histogram.plist"];
        
		NSMutableArray *contentArray = [NSMutableArray array];
		for ( NSUInteger i = 0; i < [hist count]; i++ ) {
			id x = [NSDecimalNumber numberWithDouble:i];
			id y = [NSDecimalNumber numberWithDouble:[[hist objectAtIndex:i] doubleValue]];
			[contentArray addObject:[NSMutableDictionary dictionaryWithObjectsAndKeys:x, @"x", y, @"y", nil]];
		}
		plotData = [contentArray retain];
	}
